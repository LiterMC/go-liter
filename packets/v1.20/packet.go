...
// Origin: https://wiki.vg/index.php?title=Protocol&oldid=18256
// Protocol: 763
// Protocol Name: 1.20
...
var _ Packet = (*PlayUpdateSignPkt)(nil)

func (p PlayUpdateSignPkt)Encode(b *PacketBuilder){
	b.Encode(p.Location)
	b.Bool(p.IsFrontText)
	b.String(p.Line1)
	b.String(p.Line2)
	b.String(p.Line3)
	b.String(p.Line4)
...
func (p *PlayUpdateSignPkt)DecodeFrom(r *PacketReader)(err error){
	var ok bool
	if err = p.Location.DecodeFrom(r); err != nil {
		return
	}
	if p.IsFrontText, ok = r.Bool(); !ok {
		return io.EOF
	}
...
func (p *PlayLoginPkt)DecodeFrom(r *PacketReader)(err error){
	...
	if p.PortalCooldown, ok = r.VarInt(); !ok {
		return io.EOF
	}
}
...
// ID=0x31
type PlayOpenSignEditorPkt struct {
	Location Position // Position
	/* Whether the opened editor is for the front or on the back of the sign */
	IsFrontText Bool // Boolean
}

var _ Packet = (*PlayOpenSignEditorPkt)(nil)

func (p PlayOpenSignEditorPkt)Encode(b *PacketBuilder){
	b.Encode(p.Location)
	b.Bool(p.IsFrontText)
}

func (p *PlayOpenSignEditorPkt)DecodeFrom(r *PacketReader)(err error){
	var ok bool
	if err = p.Location.DecodeFrom(r); err != nil {
		return
	}
...
// ID=0x36
type PlayEndCombatPkt struct {
	/* Length of the combat in ticks. */
	Duration VarInt // VarInt
}

var _ Packet = (*PlayEndCombatPkt)(nil)

func (p PlayEndCombatPkt)Encode(b *PacketBuilder){
	b.VarInt(p.Duration)
}

func (p *PlayEndCombatPkt)DecodeFrom(r *PacketReader)(err error){
	var ok bool
...
// ID=0x38
type PlayCombatDeathPkt struct {
...
var _ Packet = (*PlayCombatDeathPkt)(nil)

func (p PlayCombatDeathPkt)Encode(b *PacketBuilder){
	b.VarInt(p.PlayerID)
	b.JSON(p.Message)
}

func (p *PlayCombatDeathPkt)DecodeFrom(r *PacketReader)(err error){
	var ok bool
...
// ID=0x43
type PlayUpdateSectionBlocksPkt struct {
...
var _ Packet = (*PlayUpdateSectionBlocksPkt)(nil)

func (p PlayUpdateSectionBlocksPkt)Encode(b *PacketBuilder){
	b.Long(p.ChunkSectionPosition)
	b.VarInt(p.BlocksArraySize)
	TODO_Encode_Array(p.Blocks)
}

func (p *PlayUpdateSectionBlocksPkt)DecodeFrom(r *PacketReader)(err error){
	var ok bool
