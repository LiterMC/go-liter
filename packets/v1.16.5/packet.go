...
type PlayTradeListPkt struct {
	 ...
	 * | 0x26      | Play  | Client   | Trades              | Input item 2                 | Array      | Optional Slot | The second item the player has to supply for this villager trade; only present if has a second item is true.                                                                      |
	 * | 0x26      | Play  | Client   | Trades              | Trade disabled               | Array      | Boolean       | True if the trade is disabled; false if the trade is enabled.                                                                                                                     |
	 * | 0x26      | Play  | Client   | Trades              | Number of trade uses         | Array      | Integer       | Number of times the trade has been used so far. If equal to the maximum number of trades, the client will display a red X.                                                        |
	 * | 0x26      | Play  | Client   | Trades              | Maximum number of trade uses | Array      | Integer       | Number of times this trade can be used before it's exhausted.                                                                                                                     |
	 * | 0x26      | Play  | Client   | Trades              | XP                           | Array      | Integer       | Amount of XP both the player and the villager will earn each time the trade is used.                                                                                              |
	 * | 0x26      | Play  | Client   | Trades              | Special Price                | Array      | Integer       | Can be zero or negative. The number is added to the price when an item is discounted due to player reputation or other effects.                                                   |
	 ...
	 * | 0x26      | Play  | Client   | Trades              | Demand                       | Array      | Integer       | Can be zero or a positive number. Causes the price to increase when a trade is used repeatedly.                                                                                   |
...
type PlayTitlePkt struct {
	 ...
	 * | 0x4F      | Play  | Client   | 2: set action bar        | Action bar text | Chat        | Displays a message above the hotbar (the same as position 2 in Chat Message (clientbound). |
